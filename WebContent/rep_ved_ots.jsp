...
<logic:notPresent name='vedOtsAction' scope='request'>
 <logic:redirect forward='ved_ots'/>
</logic:notPresent>

<logic:notPresent name="vedOtsForm" property="action">
...
// Переход в начало списка
	if(z!=0) document.forms(0).special7.options[0].selected = true
	else
	{
	 document.forms(0).special7.options[0] = new Option("-")
	 document.forms(0).special7.options[0].value = "-"
...
<logic:present name="vedOtsForm" property="action">
<bean:define id="action" name="vedOtsForm" property="action"/>

<logic:equal name="action" value="view">
<template:insert template="<%="layouts/"+tema+"/lay/layout_simple.jsp"%>">
<template:put name="target_name">Ведомости с оценками</template:put>
...
<BR>
<p align=center>
<html:form action="/rep_ved_ots?action=report" onsubmit="return checkFields();">
...
<html:form method="post" action="/rep_ved_ots?action=null">

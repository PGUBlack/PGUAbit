...
/************************************************/
  ...
  report.write("{\\fs28 \\b1 \\qc Сведения о поступлении абитуриентов в ВУЗ по конкурсным группам (F2к) на "+StringUtil.CurrDate(".")+" г.\\line\n");
  report.write("\\par\n");
  ...
  String headline[] = new String[]{"Аббр Спец.","План приё- ма","Пода- но заявл.","Кон-курс","Платн. дого- воров","Осо-бые права","Ориг. атт.","Пол","Образование","С общ.","Инд. дост.","Занимались на курсах","Место проживания","Целе- вой прием"};
  ...
  String sub_headline[] = new String[]{"","","","","","","","М","Ж","Дневн школа","ВУЗ","ПТУ","Тех- никум","","З","С","Т","П","И","П","К","Ш","Л","Н","Пенза","Гор.П обл.","Сел.П обл.","Др. обл.",""};
  ...
  String  Kod_Kong[] = new String[250];	// Коды конкурсных групп
...
/********************************************************************/
/**** ИСКЛЮЧЕНЫ специальности: (магистры), ЮК, ФЗО, ФОЗиДО и ЦДО ****/
/********************************************************************/

      String exclude_specs = "SELECT spc.KodSpetsialnosti FROM Spetsialnosti spc, Fakultety fk WHERE spc.KodFakulteta=fk.KodFakulteta AND (spc.NazvanieSpetsialnosti LIKE '%(магистры)' OR fk.AbbreviaturaFakulteta IN ('юк','фзо','фозидо','цдо'))";
...
//      for(int i=0;i<40;i++) Summa[i] = 0; // Будет хран. суммы значений в столбцах для каждого факультета

//      for(int i=0;i<40;i++) ItogSumm[i] = 0; // Итоговые значения

//      for(int i=0;i<40;i++) ItogSumm[i] = 0; // Будет хранить содержимое колонок таблицы отчета      
...
//DOG
	stmt = conn.createStatement();
	rs = stmt.executeQuery("SELECT COUNT(s.KodSpetsialnosti), s.KodSpetsialnosti,s.Abbreviatura FROM Abiturient a,Spetsialnosti s,Konkurs kon WHERE kon.KodAbiturienta=a.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND kon.Dog LIKE 'д' AND a.DokumentyHranjatsja LIKE 'д' GROUP BY s.KodSpetsialnosti,s.Abbreviatura ORDER BY s.Abbreviatura");
	sel_ind = 0;
	while(rs.next())
	{
...
//OBSHEZHITIE
	stmt = conn.createStatement();
	rs = stmt.executeQuery("SELECT COUNT(s.KodSpetsialnosti), s.KodSpetsialnosti,s.Abbreviatura FROM Abiturient a,Spetsialnosti s,Konkurs kon WHERE kon.KodAbiturienta=a.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND NujdaetsjaVObschejitii LIKE 'д' AND a.DokumentyHranjatsja LIKE 'д' GROUP BY s.KodSpetsialnosti,s.Abbreviatura ORDER BY s.Abbreviatura");
	sel_ind = 0;
	while(rs.next())
	{
...
//************************ Всего на факультете *************************
	 if(!Kod_Kong[cur_ind-1].equals(Kod_Kong[cur_ind])) { // Если текущий код конкурсной группы не равен
                                                            // коду предыдущей то выводим строку содержащую
...
/*********************************************************************/
/************ Вывод списка конкурсных групп и специальностей *********/
/*********************************************************************/
...
String fakul[] = new String[250];
  ...
  report.write("\\fs24 \\b\\qc КОНКУРСНЫЕ ГРУППЫ И ИХ СПЕЦИАЛЬНОСТИ\n");
  stmt = conn.createStatement();

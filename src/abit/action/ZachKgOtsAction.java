...
public class ZachKgOtsAction extends Action {
    ...
    {   
	  ...
	  UserBean             user               = (UserBean)session.getAttribute("user");
        ...
        request.setAttribute( "zachKgOtsAction", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "zachKgOtsForm", form );
...
/************************************************/
    ...
    report.write("\\fs28\\b0\\qc{Абитуриенты зачисленные на очную форму обучения на бюджетной основе}\\par\n");
...
// Абитуриенты отсортированы по конкурсным группам - по возрастанию КГ и в алфавитном порядке фамилий

   stmt = conn.prepareStatement("SELECT a.KodAbiturienta,s.Abbreviatura,g.Gruppa,a.NomerLichnogoDela,a.Familija,a.Imja,a.Otchestvo,m.ShifrMedali,o.Otsenka,zo.OtsenkaEge,np.KodPredmeta,kg.Nazvanie FROM Abit_In_Prik ap,KonGruppa kg,Abiturient a,Otsenki o,ZajavlennyeShkolnyeOtsenki zo,Spetsialnosti s,Medali m,Gruppy g,NazvanijaPredmetov np,Forma_Obuch fo WHERE ap.KodAbiturienta=a.KodAbiturienta AND fo.KodFormyOb=a.KodFormyOb AND np.KodPredmeta=o.KodPredmeta AND np.KodPredmeta=zo.KodPredmeta AND zo.KodAbiturienta=a.KodAbiturienta AND zo.KodPredmeta=o.KodPredmeta AND a.KodAbiturienta=o.KodAbiturienta AND a.KodMedali=m.KodMedali AND a.KodSpetsialnZach=s.KodSpetsialnosti AND a.KodGruppy=g.KodGruppy AND kg.KodKonGruppy=s.KodKonGruppy AND o.KodPredmeta IN(SELECT KodPredmeta FROM EkzamenyNaSpetsialnosti WHERE KodSpetsialnosti LIKE a.KodSpetsialnZach) AND a.KodVuza LIKE ? AND NomerPlatnogoDogovora IS NULL AND fo.Sokr IN ('очная') GROUP BY a.KodAbiturienta,kg.Nazvanie,s.Abbreviatura,g.Gruppa,a.NomerLichnogoDela,a.Familija,a.Imja,a.Otchestvo,m.ShifrMedali,o.Otsenka,zo.OtsenkaEge,np.KodPredmeta,g.KodGruppy ORDER BY kg.Nazvanie ASC,Familija,Imja,Otchestvo,a.KodAbiturienta,np.KodPredmeta ASC");
   stmt.setObject(1,session.getAttribute("kVuza"),Types.INTEGER); 
   rs = stmt.executeQuery();
   while (rs.next()) {

// Вывод названия конкурсной группы

     if(rs.getInt(12) != old_KG) {

    if(old_KG != -1) report.write("\\pard");

    report.write("\\b1\\par\\ql\\fs24{Конкурсная группа №"+rs.getInt(12)+"}\\b0\\fs22\n");

    report.write("\\par\n");
    report.write("\\qc\\fs22 \\trowd \\trhdr \\trqc\\trgaph108\\trrh280\\trleft30\n");

...
/************************************************/
  ...
  report.write("{\\fs28 \\b1 \\qc Сведения о поступлении абитуриентов в ВУЗ по факультетам (F2) на "+StringUtil.CurrDate(".")+" г.\\line\n");
  report.write("\\par\n");
  ...
  String headline[] = new String[]{"Аббр Спец.","План приё- ма", "План приё- ма по льго- там", "План приё- ма по дог.","Пода- но заявл.","Кон-курс","Платн. дого- воров","Льготники","Ориг. атт.","Пол","Образование","С общ.","Отличия","Место проживания","Целе- вой прием"};
 ...
 // report.write("\\clvertalc \\clbrdrb\\brdrs \\brdrdb\\brdrcf1 \\clbrdrl\\brdrs\\brdrw20\\brdrcf1 \\clbrdrr\\brdrs\\brdrw20\\brdrcf1 \\cellx13050\n");
  ...
  String sub_headline[] = new String[]{"","","","","","","","","","М","Ж","Дневн школа","ВУЗ","ПТУ","Тех- никум","","З","С","Т","П","Пенза","Гор.П обл.","Сел.П обл.","Др. обл.",""};
...
/********************************************************************/
    ...
    String exclude_specs = "SELECT spc.KodSpetsialnosti FROM Spetsialnosti spc, Fakultety fk WHERE spc.KodFakulteta=fk.KodFakulteta AND (spc.NazvanieSpetsialnosti LIKE '%(магистры)' OR fk.AbbreviaturaFakulteta IN ('юк','фзо','фозидо','цдо', 'ЗФПО', 'ФМС', 'иито', 'АСП','ОРД','ИНТ','ИМС'))";

    stmt = conn.createStatement();
    rs = stmt.executeQuery("SELECT f.AbbreviaturaFakulteta, s.Abbreviatura, s.KodSpetsialnosti FROM Fakultety f, Spetsialnosti s WHERE s.KodFakulteta=f.KodFakulteta and s.eduLevel in ('б', 'с') and tip_spec = 'о'  AND s.KodSpetsialnosti NOT IN ("+exclude_specs+")  ORDER BY s.Abbreviatura ");
    while(rs.next())
    {

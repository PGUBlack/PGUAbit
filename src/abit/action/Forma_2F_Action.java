...
public class Forma_2F_Action extends Action {
    ...
    {   
        ...
        request.setAttribute( "forma_2f_Action", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "forma_2f_Form", form );
...
/************************************************/
  ...
  String name = "Форма №2Ф";

  String file_con = "forma2F_"+StringUtil.CurrDate("-");
  ...
  report.write("{\\fs28 \\b1 \\qc Сведения о поступлении абитуриентов в ВУЗ по факультетам (F2F) на "+StringUtil.CurrDate(".")+" г.\\line\n");
  report.write("\\fs12\\par\n");
...
//BUDJETNIKI
	stmt = conn.createStatement();
	rs = stmt.executeQuery("SELECT COUNT(s.KodSpetsialnosti), s.KodSpetsialnosti,s.Abbreviatura FROM Abiturient a,Spetsialnosti s,Forma_Obuch fo,Konkurs kon WHERE kon.KodAbiturienta=a.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND kon.Bud LIKE 'д' AND a.KodFormyOb=fo.KodFormyOb AND fo.Sokr LIKE 'заочная' AND a.DokumentyHranjatsja LIKE 'д' GROUP BY s.KodSpetsialnosti,s.Abbreviatura ORDER BY s.Abbreviatura");
	sel_ind = 0;
	while(rs.next())
	{
...
report.write("}\\pard{\n");
  stmt = conn.createStatement();
  rs = stmt.executeQuery("SELECT f.KodFakulteta, f.Fakultet, f.AbbreviaturaFakulteta FROM Fakultety f, Spetsialnosti s WHERE s.KodFakulteta=f.KodFakulteta AND s.KodSpetsialnosti IN ("+include_specs+") GROUP BY f.Fakultet,f.KodFakulteta, f.AbbreviaturaFakulteta ORDER BY f.Fakultet");
  int stfak = 0;
  while(rs.next())
  {
  ...
  while(stfak<=stfak1)
  {
    ...
    while(rs.next())
    {
     report.write("\\par\\fs20\\b0\\ql{"+rs.getString(1)+"	"+rs.getString(2)+"}\n");
    }
    stfak++;
  }

...
/************************************************/
  ...
  String name = "Проходной балл по специальностям на "+StringUtil.CurrDate("-");

  String file_con = "info_abit_"+StringUtil.CurrDate("-");

  if(abit_SD.getSpecial1().equals("%")) name += " с учетом всех приор.";

  else name += " с учетом "+abit_SD.getSpecial1()+"-го приор.";
  ...
  if(rs.next()) 
    report.write("\\fs28\\b1\\qc Количество заявлений, поданных абитуриентами\\par в {"+rs.getString(1)+"} на {"+StringUtil.CurrDate(".")+"г.}\\par\n");
  ...
  report.write("\\intbl\\fs22\\qc{Количество бюдж. мест\\par(план приема)}\\cell\n");
  report.write("\\intbl\\fs18\\par\\fs22\\qc{Подано заявлений на очную форму обучения}\\fs18\\par\\cell\n");
  report.write("\\intbl\\fs22\\qc{Текущий}\\par{конкурсный балл}\\cell\n");
  ...
  report.write("\\intbl\\b1\\fs22\\qc{по заявл.}\\cell\n");
  report.write("\\intbl\\fs22\\qc{в т.ч. по}\\par{оригиналам}\\par{документов}\\b0\\cell\n");
  report.write("\\intbl\\b0\\row\n");
...
// Количество оригиналов документов об образовании
    ...
    stmt = conn.prepareStatement("SELECT COUNT(a.KodAbiturienta) FROM Abiturient a, Konkurs kon, Spetsialnosti s WHERE a.KodAbiturienta=kon.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND a.DokumentyHranjatsja LIKE 'д' AND a.TipDokSredObraz LIKE 'о' AND kon.KodSpetsialnosti LIKE ? AND kon.Prioritet LIKE ?");
...
//  По оригиналам и копиям документов об образовании

    stmt = conn.prepareStatement("SELECT a.KodAbiturienta,SUM(oa.OtsenkaEge)\"SummaEge\" FROM Abiturient a,Oa oa, Konkurs kon, Spetsialnosti s, Medali m, NazvanijaPredmetov np,Kursy k,EkzamenyNaSpetsialnosti ens WHERE k.KodKursov=a.KodKursov AND a.KodAbiturienta=oa.KodAbiturienta AND a.KodAbiturienta=kon.KodAbiturienta AND a.KodMedali=m.KodMedali AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND kon.KodSpetsialnosti=ens.KodSpetsialnosti AND oa.KodPredmeta=np.KodPredmeta AND a.KodAbiturienta=oa.KodAbiturienta AND kon.KodSpetsialnosti LIKE ? AND ens.KodPredmeta=oa.KodPredmeta AND a.DokumentyHranjatsja LIKE 'д' AND kon.Prioritet LIKE ? GROUP BY a.KodAbiturienta,a.TipDokSredObraz ORDER BY SummaEge DESC");
    stmt.setObject(1,rs.getString(5),Types.INTEGER);
    stmt.setObject(2,abit_SD.getSpecial1(),Types.VARCHAR);
    rs2 = stmt.executeQuery();
    while(rs2.next()){

...
public class Forma_6_Action extends Action {
    ...
    {   
        ...
        request.setAttribute( "forma_6_Action", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "forma_6_Form", form );
...
/************************************************/
    ...
    String file_con = "forma6_"+StringUtil.CurrDate("-");
  ...
  report.write("\\fs28 \\b1 \\qc Сведения о зачислении (F6) на "+StringUtil.CurrDate(".")+" г. по факультетам\\line\n");
  report.write("\\par\n");
...
/********************************************************************/
	...
	rs = stmt.executeQuery("SELECT f.AbbreviaturaFakulteta, s.Abbreviatura, s.KodSpetsialnosti FROM Fakultety f, Spetsialnosti s WHERE s.KodFakulteta=f.KodFakulteta AND s.KodSpetsialnosti NOT IN ("+exclude_specs+") ORDER BY s.Abbreviatura");
	while(rs.next())
	{
...
//********************* 
  ...
  return mapping.findForward("rep_brw");
        ...
        if(rep_forma_6_f) return mapping.findForward("rep_forma_6_f");

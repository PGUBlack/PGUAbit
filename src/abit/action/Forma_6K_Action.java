...
public class Forma_6K_Action extends Action {
    ...
    {   
        ...
        request.setAttribute( "forma_6k_Action", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "forma_6k_Form", form );
...
/************************************************/
    ...
    String name = "Форма №6К";

    String file_con = "forma6k_"+StringUtil.CurrDate("-");
  ...
  report.write("\\fs28 \\b1 \\qc Сведения о зачислении (F6к) на "+StringUtil.CurrDate(".")+" г. по факультетам\\line\n");
  report.write("\\par\n");
...
/********************************************************************/
/**** ИСКЛЮЧЕНЫ специальности: (магистры), ЮК, ФОЗиДО, ФЗО и ЦДО ****/
/********************************************************************/
	...
	stmt = conn.createStatement();
        rs = stmt.executeQuery("SELECT kg.Nazvanie, s.Abbreviatura, s.KodSpetsialnosti, kg.KodKonGruppy FROM KonGruppa kg, Spetsialnosti s,Fakultety f WHERE f.KodFakulteta=s.KodFakulteta AND s.KodKonGruppy=kg.KodKonGruppy AND s.KodSpetsialnosti NOT IN ("+exclude_specs+") ORDER BY kg.KodKonGruppy");
	while(rs.next())
	{
...
//********************* 
  ...
  return mapping.findForward("rep_brw");
        ...
        if(rep_forma_6k_f) return mapping.findForward("rep_forma_6k_f");

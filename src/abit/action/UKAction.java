...
public class UKAction extends Action{
    ...
    {   
        ...
        request.setAttribute( "uKAction", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "ukForm", form );
...
if(abit_SD.getKodFakulteta()!=0){
  ...
  String name = "Список подавших заявление в магистратуру(предв.) "+AF;

  String file_con = "lists_mag_"+StringUtil.toEng(AF)+"_predv_first_wave_allPr";
...
/**********************************************************/
  ...
  query = new StringBuffer("SELECT DISTINCT f.AbbreviaturaFakulteta,s.ShifrSpetsialnosti,s.Abbreviatura,s.NazvanieSpetsialnosti,s.KodSpetsialnosti,s.PlanPriema,s.TselPr_PGU,s.TselPr_ROS,s.PlanPriemaDog,f.kodFakulteta FROM Fakultety f,Spetsialnosti s, Konkurs kon, Abiturient a WHERE f.KodFakulteta=s.KodFakulteta AND a.KodAbiturienta=kon.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND s.KodFakulteta LIKE '"+KF+"' AND s.edulevel LIKE 'м'");
...
// По СУММЕ НАБРАННЫХ БАЛЛОВ
    ...
    oldBallAbt = -1;
       ...
       query1 = new StringBuffer("SELECT DISTINCT a.KodAbiturienta,kon.nomerlichnogodela,a.Familija,a.Imja,a.Otchestvo,a.TipDokSredObraz,l.ShifrLgot,kon.PR,a.viddoksredobraz,kon.prof,kon.olimp FROM Abiturient a, Spetsialnosti s, lgoty l,TselevojPriem tp, Konkurs kon WHERE kon.op=l.kodlgot AND kon.KodAbiturienta=a.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND a.DokumentyHranjatsja LIKE 'д' AND kon.KodSpetsialnosti LIKE '"+rs2.getString(5)+"' AND a.Prinjat not in ('1','2','3','4','5','7','д') AND a.KodAbiturienta NOT IN ("+excludeList.toString()+") ");
...
summa=StringUtil.toInt(rs10.getString(6), 0) + StringUtil.toInt(rs10.getString(8),0);

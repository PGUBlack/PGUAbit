	  ...
	  PackageData packageData = new PackageData();
		 ...
		 PackageData.AdmissionInfo.DistributedAdmissionVolume dvolume = new PackageData.AdmissionInfo.DistributedAdmissionVolume();
           ...
           admissioninfo.setDistributedAdmissionVolume(dvolume);
					...
					int flag = 0;
					stmt = conn
							.prepareStatement("select s.kodspetsialnosti,s.nazvaniespetsialnosti, s.tip_spec,s.edulevel, s.fisid, s.planpriema, s.planpriemadog, s.planpriemalg, s.TselPr_PGU,s.TselPr_ROS,s.TselPr_1,s.TselPr_2,s.TselPr_3,(s.krim_obshee+s.krim_cp+s.krim_ok) from spetsialnosti s where s.edulevel in ('б','с')");
					rs = stmt.executeQuery();
					while (rs.next()) {
						flag = 0;

...
public class ORDAction extends Action{
    ...
    {   
        ...
        request.setAttribute( "ordAction", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "ordForm", form );

/*****************  Возврат к предыдущей странице   *******************/
          if(us.quit("exit")) return mapping.findForward("back");
/**********************************************************************/
          
          pstmt = conn.prepareStatement("SELECT DISTINCT KodFakulteta,AbbreviaturaFakulteta FROM Fakultety WHERE kodfakulteta=30 and KodVuza LIKE ? ORDER BY 2 ASC");
...
if(abit_SD.getKodFakulteta()!=0){
  ...
  String name = "Список подавших заявление в ординатуру(предв.) "+AF;

  String file_con = "lists_ord_"+StringUtil.toEng(AF)+"_predv_first_wave_allPr";
...
/**********************************************************/
  ...
  query = new StringBuffer("SELECT DISTINCT f.AbbreviaturaFakulteta,s.ShifrSpetsialnosti,s.Abbreviatura,s.NazvanieSpetsialnosti,s.KodSpetsialnosti,s.PlanPriema,s.TselPr_PGU,s.TselPr_ROS,s.PlanPriemaDog,f.kodFakulteta FROM Fakultety f,Spetsialnosti s, Konkurs kon, Abiturient a WHERE f.KodFakulteta=s.KodFakulteta AND a.KodAbiturienta=kon.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND s.KodFakulteta LIKE '"+KF+"' AND s.edulevel IN('о') ");
    ...
    if( rs.next() && StringUtil.toInt(rs.getString(1),0) != 0 ) {
    	 ...
    	 header = false;
    	    ...
    	    oldBallAbt = -1;
    	       ...
    	       query1 = new StringBuffer("SELECT DISTINCT a.KodAbiturienta,kon.nomerlichnogodela,a.Familija,a.Imja,a.Otchestvo,a.TipDokSredObraz,l.ShifrLgot,kon.PR,a.viddoksredobraz,kon.prof,kon.olimp FROM Abiturient a, Spetsialnosti s, lgoty l, Konkurs kon WHERE kon.op=l.kodlgot AND kon.KodAbiturienta=a.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND a.DokumentyHranjatsja LIKE 'д' AND kon.KodSpetsialnosti LIKE '"+rs2.getString(5)+"' AND a.Prinjat not in ('1','2','3','4','5','7','д') and kon.target=5 AND a.KodAbiturienta NOT IN ("+excludeList.toString()+") ");
    	...
    	query1.append(" GROUP BY a.KodAbiturienta,kon.nomerlichnogodela,a.Familija,a.Imja,a.Otchestvo,a.TipDokSredObraz,l.ShifrLgot,kon.PR,a.viddoksredobraz,kon.prof,kon.olimp ORDER BY a.KodAbiturienta,a.Familija,a.Imja,a.Otchestvo,l.ShifrLgot");
    	...
    	//System.out.println(query);
    	    ...
    	    while(rs9.next()) {
    	    	stmt10 = conn.prepareStatement("INSERT INTO Forsen(N,Shifr,F,I,O,ko,op,pp,sum,ka,vd,ind) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
    	    	N++;
    	   ...
    	   //     report.write("\\clvertalc \\clbrdrb\\brdrs \\brdrdb\\brdrcf1 \\clbrdrl\\brdrs\\brdrw20\\brdrcf1 \\clbrdrr\\brdrs\\brdrw20\\brdrcf1 \\cellx500\n"); //Nomer
    	      ...
    	      stmt10 = conn.prepareStatement("SELECT Shifr,F,I,O,ko,sum,pp,ind FROM Forsen ORDER BY sum DESC");
...
// По СУММЕ НАБРАННЫХ БАЛЛОВ
    ...
    oldBallAbt = -1;
       ...
       query1 = new StringBuffer("SELECT DISTINCT a.KodAbiturienta,kon.nomerlichnogodela,a.Familija,a.Imja,a.Otchestvo,a.TipDokSredObraz,l.ShifrLgot,kon.PR,a.viddoksredobraz,kon.prof,kon.olimp FROM Abiturient a, Spetsialnosti s, lgoty l, Konkurs kon WHERE kon.op=l.kodlgot AND kon.KodAbiturienta=a.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti and kon.target=1 AND a.DokumentyHranjatsja LIKE 'д' AND kon.KodSpetsialnosti LIKE '"+rs2.getString(5)+"' AND a.Prinjat not in ('1','2','3','4','5','7','д') AND a.KodAbiturienta NOT IN ("+excludeList.toString()+") ");

...
public class ListsKgEgeBallAction extends Action {
    ...
    {   
        ...
        request.setAttribute( "listsKgEgeBallAction", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "listsKgEgeBallForm", form );
...
/**********************************************************************/
  ...
  name = "Список абит-в "+Abbr_Vuza+" по конк. гр. с баллом (ЕГЭ)";
  file_con = "lists_kgegeball_"+StringUtil.toEng(Abbr_Vuza)+"_ots";
...
/************************************************/
  ...
  String file_name = (request.getRealPath(request.getContextPath())).substring(0,request.getRealPath(request.getContextPath()).lastIndexOf('\\'))+Constants.RELATIVE_PATH+"\\"+Constants.RELATIVE_PATH+"\\"+((ReportsBrowserBean)session.getAttribute("rpt")).getFileName();
...
// Перебор конкурсных групп
  ...
  while(rs_a.next()){
    ...
    report.write("\\par\n");
    report.write("\\fs28\\b1\\qc Список абитуриентов с баллами ЕГЭ конкурсной группы №"+rs_a.getString(2)+"\n");
    report.write("\\b0\\par\\par\n");

// МЕДАЛИСТЫ

    num = 0;
    stmt = conn.prepareStatement("SELECT a.KodAbiturienta,a.Familija,a.Imja,a.Otchestvo,a.NomerLichnogoDela,TipDokSredObraz,SUM(CONVERT(int,zo.OtsenkaEge)),k.ShifrKursov FROM Kursy k, ZajavlennyeShkolnyeOtsenki zo, Spetsialnosti s,Abiturient a,KonGruppa kg, Medali m,EkzamenyNaSpetsialnosti ens WHERE k.KodKursov=a.KodKursov AND ens.KodPredmeta=zo.KodPredmeta AND ens.KodSpetsialnosti=s.KodSpetsialnosti AND m.KodMedali=a.KodMedali AND a.KodAbiturienta = zo.KodAbiturienta AND DokumentyHranjatsja LIKE 'д' AND m.ShifrMedali NOT LIKE 'н' AND a.KodVuza LIKE ? AND s.KodKonGruppy=kg.KodKonGruppy AND s.KodSpetsialnosti=a.KodSpetsialnosti AND kg.KodKonGruppy LIKE ? GROUP BY a.KodAbiturienta,a.NomerLichnogoDela,a.Familija,a.Imja,a.Otchestvo,a.TipDokSredObraz,m.ShifrMedali,k.ShifrKursov ORDER BY 7 DESC,Familija,Imja,Otchestvo,m.ShifrMedali,NomerLichnogoDela ASC");
    stmt.setObject(1,session.getAttribute("kVuza"),Types.INTEGER);

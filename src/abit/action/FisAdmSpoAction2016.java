...
public class FisAdmSpoAction2016 extends Action{
  ...
  conn = us.getConn(user.getSid());
	  ...
	  PackageData packageData = new PackageData();
		 ...
		 PackageData.AdmissionInfo.DistributedAdmissionVolume dvolume = new PackageData.AdmissionInfo.DistributedAdmissionVolume();
		 	...
		 	stmt = conn.prepareStatement("select distinct s.shifrspetsialnosti, s.edulevel,dc.id from spetsialnosti s, dictionarycode10 dc where s.edulevel in ('п') and s.shifrspetsialnosti like dc.code and s.kodfakulteta=36");
			...
			while (rs.next()) {
        	   PackageData.AdmissionInfo.DistributedAdmissionVolume.Item ditem = new PackageData.AdmissionInfo.DistributedAdmissionVolume.Item();
      		 	PackageData.AdmissionInfo.AdmissionVolume.Item litem = new PackageData.AdmissionInfo.AdmissionVolume.Item();
        	          	   ...
        	          	   litem.setUID("16-"+rs.getString(1)+"s");
        	          	   ditem.setAdmissionVolumeUID("16d-"+rs.getString(1));
        	          	   ditem.setLevelBudget(1);
        	          	   litem.setCampaignUID("16s-1");
        	          	   
        	          		   litem.setEducationLevelID(17);
        	          	   
        	        
        	          	   litem.setDirectionID(rs.getInt(3));
        	          	 stmt2 = conn.prepareStatement("select shifrspetsialnosti, edulevel,'-', sum(planpriema)-sum(krim_ok)-sum(planpriemalg), sum(planpriemadog), sum(TselPr_ROS)+sum(krim_cp), sum(planpriemalg)+sum(krim_ok) from spetsialnosti where shifrspetsialnosti like ? and tip_spec like 'о' and edulevel in ('п')  and kodfakulteta=35 group by shifrspetsialnosti,edulevel");
        	             ...
        	             stmt2 = conn.prepareStatement("select shifrspetsialnosti, edulevel,'-', sum(planpriema)-sum(krim_ok)-sum(planpriemalg), sum(planpriemadog), sum(TselPr_ROS)+sum(krim_cp), sum(planpriemalg)+sum(krim_ok) from spetsialnosti where shifrspetsialnosti like ? and tip_spec like 'з' and edulevel in ('п') and kodfakulteta=35 group by shifrspetsialnosti,edulevel");
        	          	 stmt2.setObject(1,rs.getString(1),Types.VARCHAR);
        	          	 rs2 = stmt2.executeQuery();
        	             if (rs2.next()) {
        	             ...
        	             stmt2 = conn.prepareStatement("select shifrspetsialnosti, edulevel,'-', sum(planpriema)-sum(krim_ok)-sum(planpriemalg), sum(planpriemadog), sum(TselPr_ROS)+sum(krim_cp), sum(planpriemalg)+sum(krim_ok) from spetsialnosti where shifrspetsialnosti like ? and tip_spec like 'в' and edulevel in ('п') and kodfakulteta=35 group by shifrspetsialnosti,edulevel");
        	          	 stmt2.setObject(1,rs.getString(1),Types.VARCHAR);
        	          	 rs2 = stmt2.executeQuery();
        	             if (rs2.next()) {
       ...
       //    admissioninfo.setDistributedAdmissionVolume(dvolume);
   		...
   		//--------------------------------------------------------------------------
					...
					int flag = 0;
					stmt = conn
							.prepareStatement("select s.kodspetsialnosti,s.nazvaniespetsialnosti, s.tip_spec,s.edulevel, dc10.id, (s.planpriema-s.krim_obshee-s.tselpr_ros-s.planpriemalg), (s.planpriemadog-s.ppkrimdog-s.planpriemaig), (s.planpriemalg), s.TselPr_ROS, (s.krim_obshee-s.krim_ok-s.krim_cp), s.ppkrimdog, s.krim_ok, s.krim_cp,s.planpriemaig from spetsialnosti s, dictionarycode10 dc10 where s.edulevel in ('п') and s.kodfakulteta=35 and s.shifrspetsialnosti=dc10.code");
					rs = stmt.executeQuery();
					while (rs.next()) {
						flag = 0;
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(6)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "bs");
						...
						ep.setUID("cgi16-"+rs.getString(2)+""+rs.getString(3)+"bs");
						eps.getEduProgram().add(ep);
						cg.setEduPrograms(eps);
						
						
						
						cg.setUID("cg16-" + rs.getString(1) + "bs");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "bs");
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(7)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "ds");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						cg.setUID("cg16-" + rs.getString(1) + "ds");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "ds");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(8)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "ls");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram ep = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram();
						ep.setName(rs.getString(2));
						ep.setUID("cgi16-"+rs.getString(2)+""+rs.getString(3)+"ls");
						eps.getEduProgram().add(ep);
						cg.setEduPrograms(eps);
						
						cg.setUID("cg16-" + rs.getString(1) + "ls");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "ls");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(9)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "ts");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram ep = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram();
						ep.setName(rs.getString(2));
						ep.setUID("cgi16-"+rs.getString(2)+""+rs.getString(3)+"ts");
						eps.getEduProgram().add(ep);
						cg.setEduPrograms(eps);
						
						cg.setUID("cg16-" + rs.getString(1) + "ts");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "ts");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
...
//--------------------------------------------------------------------------	
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(10)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "kbs");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram ep = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram();
						ep.setName(rs.getString(2));
						ep.setUID("cgi16-"+rs.getString(2)+""+rs.getString(3)+"kbs");
						eps.getEduProgram().add(ep);
						cg.setEduPrograms(eps);
						
						
						cg.setUID("cg16-" + rs.getString(1) + "kbs");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "kbs");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(11)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "kds");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						
						cg.setUID("cg16-" + rs.getString(1) + "kds");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "kds");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(12)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "kls");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram ep = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram();
						ep.setName(rs.getString(2));
						ep.setUID("cgi16-"+rs.getString(2)+""+rs.getString(3)+"kls");
						eps.getEduProgram().add(ep);
						cg.setEduPrograms(eps);
						cg.setIsForKrym(true);
						
						cg.setUID("cg16-" + rs.getString(1) + "kls");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "kls");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(13)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "kts");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						cg.setUID("cg16-" + rs.getString(1) + "kts");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "kts");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
					
							cg.setEducationLevelID(14);
						...
						//--------------------------------------------------------------------------
						if(rs.getInt(14)>0){
							dcg.getCompetitiveGroupUID().add("cg16-" + rs.getString(1) + "digs");
						//--------------------------------------------------------------------------
						//-----------------------Предметы конкурса----------------------------------
						//--------------------------------------------------------------------------
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup cg = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup();
						...
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram ep = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.EduPrograms.EduProgram();
						ep.setName(rs.getString(2));
						ep.setUID("cgi16-"+rs.getString(2)+""+rs.getString(3)+"digs");
						eps.getEduProgram().add(ep);
						cg.setEduPrograms(eps);
						
						cg.setUID("cg16-" + rs.getString(1) + "digs");
						cg.setCampaignUID("16s-1");
						cg.setName("16-" + rs.getString(2) + "-"
								+ rs.getString(1) + "" + rs.getString(3) + "digs");
						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem cgis = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.CompetitiveGroupItem();
...
/*						PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.TargetOrganizations t = new PackageData.AdmissionInfo.CompetitiveGroups.CompetitiveGroup.TargetOrganizations();
  		...
  		File of = new File("D:/admissionSPO.xml");
  		...
  		of = new File("D:/admissionSPODelete.xml");
  		os = new FileOutputStream(of);
  		// Маршаллизация

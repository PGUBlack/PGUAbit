...
public class Forma_6Z_Action extends Action {
    ...
    {   
        ...
        request.setAttribute( "forma_6z_Action", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "forma_6z_Form", form );
...
/************************************************/
    ...
    String name = "Форма №6z";

    String file_con = "forma6z_"+StringUtil.CurrDate("-");
  ...
  report.write("\\fs28 \\b1 \\qc Сведения о зачислении (F6z) на "+StringUtil.CurrDate(".")+" г. по факультетам\\line\n");
  report.write("\\par\n");
...
/******************************************/
      ...
      String include_faks = "'юк','фзо','фозидо'";
...
//PLATN_DOGOVOR
      stmt = conn.createStatement();
      rs = stmt.executeQuery("SELECT COUNT(a.KodSpetsialnZach), a.KodSpetsialnZach FROM Abiturient a WHERE a.KodAbiturienta IN(SELECT KodAbiturienta FROM Konkurs WHERE Dog_Ok LIKE 'д') AND a.NomerPlatnogoDogovora IS NOT NULL AND a.Prinjat IN ('1','2','3','4','д') AND a.DokumentyHranjatsja LIKE 'д' GROUP BY a.KodSpetsialnZach ORDER BY a.KodSpetsialnZach");
      sel_ind = 0;
      while(rs.next())
      {
...
//********************* 
  ...
  return mapping.findForward("rep_brw");
        ...
        if(rep_forma_6z_f) return mapping.findForward("rep_forma_6z_f");

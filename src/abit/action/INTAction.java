...
public class INTAction extends Action{
    ...
    {   
        ...
        request.setAttribute( "intAction", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "intForm", form );

/*****************  Возврат к предыдущей странице   *******************/
          if(us.quit("exit")) return mapping.findForward("back");
/**********************************************************************/
          
          pstmt = conn.prepareStatement("SELECT DISTINCT KodFakulteta,AbbreviaturaFakulteta FROM Fakultety WHERE kodfakulteta=31 and KodVuza LIKE ? ORDER BY 2 ASC");
...
if(abit_SD.getKodFakulteta()!=0){
  ...
  String name = "Список подавших заявление в интернатуру(предв.) "+AF;

  String file_con = "lists_int_"+StringUtil.toEng(AF)+"_predv_first_wave_allPr";
...
/**********************************************************/
  ...
  query = new StringBuffer("SELECT DISTINCT f.AbbreviaturaFakulteta,s.ShifrSpetsialnosti,s.Abbreviatura,s.NazvanieSpetsialnosti,s.KodSpetsialnosti,s.PlanPriema,s.TselPr_PGU,s.TselPr_ROS,s.PlanPriemaDog,f.kodFakulteta FROM Fakultety f,Spetsialnosti s, Konkurs kon, Abiturient a WHERE f.KodFakulteta=s.KodFakulteta AND a.KodAbiturienta=kon.KodAbiturienta AND kon.KodSpetsialnosti=s.KodSpetsialnosti AND s.KodFakulteta LIKE '"+KF+"' AND s.edulevel IN('и') ");

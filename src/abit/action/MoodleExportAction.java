...
//				 stmt = conn.prepareStatement("select a.kodabiturienta, z.kodpredmeta, a.datainput from abit.dbo.abiturient A, abit.dbo.ZajavlennyeShkolnyeOtsenki z where a.kodabiturienta = z.kodabiturienta and a.datainput = '"+currdate+"'");
	          ...
	          /*  stmt = conn.prepareStatement("select kodgruppy, gruppa from gruppy");
				 ...
				 stmt = conn.prepareStatement("select distinct a.kodabiturienta, a.nomerlichnogodela,  a.familija, a.imja, a.otchestvo, a.kodgruppy, lang=case adi.vstup WHEN 'Английский язык' then 'en' else 'ru' end, a.InostrannyjJazyk from abiturient a, abitDopInf adi, zajavlennyeshkolnyeotsenki zso where a.kodabiturienta=zso.kodabiturienta and zso.examen like '+' and adi.kodabiturienta = a.kodabiturienta and dokumentyHranjatsja = 'д' and a.kodabiturienta>73 and a.kodgruppy in (359,364,372,376,377,384,386,389,390,392)");

...
public class BallPotokAction extends Action {
    ...
    {   
        ...
        request.setAttribute( "ballPotokAction", new Boolean(true) );
...
/*********  Получение соединения с БД и ведение статистики  ***********/
          ...
          request.setAttribute( "ballPotokForm", form );
...
/************************************************************************************************/
       ...
       if(form.getAction()==null || form.getAction().equals("show")) { 
         ...
         stmt_a = conn.prepareStatement("SELECT AbbreviaturaFakulteta,KodFakulteta FROM Fakultety ORDER BY KodFakulteta ASC");
         ...
         while(rs_a.next()) {
           ...
           stmt = conn.prepareStatement("SELECT SUM(CONVERT(int,zso.OtsenkaEge)),Familija,a.KodAbiturienta FROM EkzamenyNaSpetsialnosti ens,Fakultety f,Spetsialnosti s,Abiturient a,ZajavlennyeShkolnyeOtsenki zso,Gruppy g WHERE ens.KodPredmeta=zso.KodPredmeta AND ens.KodSpetsialnosti=a.KodSpetsialnosti AND g.KodGruppy=a.KodGruppy AND a.KodAbiturienta=zso.KodAbiturienta AND a.KodSpetsialnosti=s.KodSpetsialnosti AND s.KodFakulteta=f.KodFakulteta AND g.Potok LIKE ? AND f.KodFakulteta LIKE ? GROUP BY a.KodAbiturienta,a.Familija ORDER BY 1 DESC");
           ...
           for(int i=300;i>=0;i--) {
              ...
              if(kol_abits[i] != 0)
                tmp.setSpecial1(""+kol_abits[i]);
              else
                tmp.setSpecial1("");
              list.add(tmp);
           }
           tmp_ots.setList(list);
           tmp_ots.setAbbreviaturaFakulteta(rs_a.getString(1));
...
/************************************************/
    ...
    String file_con = new String("sum_ball_pot_"+session.getAttribute("nomPot"));
  ...
  for(int balls=300; balls>0; balls-=20) {
      ...
      report.write("\\qc");
      report.write("\\intbl\\b1 Факультет\\cell\n");

...
public class AbitSrchOnlineAction extends Action {
...
/**********************************************************************/
        ...
        //    if ( form.getAction() == null ) form.setAction(us.getClientIntName("search","init"));
          if ( form.getAction() == null ) form.setAction("search");
...
// Обработка последовательности значений запроса для признака зачисления
   ...
   session.setAttribute("NujdaetsjaVObschejitii",StringUtil.toMySQL((abit_A.getNujdaetsjaVObschejitii()+"")));
   ...
   session.setAttribute("TrudovajaDejatelnost",StringUtil.toMySQL((abit_A.getTrudovajaDejatelnost()+"")));
   ...
   session.setAttribute("Sobesedovanie",StringUtil.toMySQL((abit_A.getSobesedovanie()+"")));
...
/****************************** Выборка абитуриентов из БД ******************************/
                    ...
                    query.append(",Familija,Imja,Otchestvo,ShifrKursov,ShifrMedali,ShifrLgot,NomerPlatnogoDogovora,DataRojdenija,Pol,SrokObuchenija,GodOkonchanijaSrObrazovanija,GdePoluchilSrObrazovanie,NomerShkoly,InostrannyjJazyk,NujdaetsjaVObschejitii,Grajdanstvo,Punkty.Nazvanie,Rajony.NazvanieRajona,Oblasti.NazvanieOblasti,PolnoeNaimenovanieZavedenija,TipOkonchennogoZavedenija,TrudovajaDejatelnost,Gruppa,NapravlenieOtPredprijatija,TipDokumenta,NomerDokumenta,SeriaDokumenta,DataVydDokumenta,KemVydDokument,TipDokSredObraz,Abiturient.Sobesedovanie,TselevojPriem.ShifrPriema,NomerSertifikata,KopijaSertifikata,Ball,Prinjat,KodSpetsialnZach,Forma_Obuch.Sokr,Osnova_Obuch.Sokr,SeriaAtt,NomerAtt,Spetsialnosti.KodSpetsialnosti,PR.PreemptiveRight,AbitDopInf.ProvidingSpecialConditions,AbitDopInf.Address,AbitDopInf.returndocument FROM Forma_Obuch,Osnova_Obuch,TselevojPriem,Abiturient,Lgoty,Medali,Kursy,Spetsialnosti,Fakultety,Punkty,Rajony,Oblasti,Zavedenija,Gruppy,PR,AbitDopInf");/////
                    ...
                    if((""+session.getAttribute("UseAllSpecs")).equals("yes"))
                      query.append("Konkurs.KodAbiturienta=Abiturient.KodAbiturienta AND Konkurs.KodSpetsialnosti=Spetsialnosti.KodSpetsialnosti AND ");//AND PreemptiveRight.KodAbiturienta=Abiturient.KodAbiturienta AND AbitDopInf.KodAbiturienta=Abiturient.KodAbiturienta 
                    else
                      query.append("Abiturient.KodSpetsialnosti = Spetsialnosti.KodSpetsialnosti AND ");/////////////////////AND PreemptiveRight.KodAbiturienta=Abiturient.KodAbiturienta AND AbitDopInf.KodAbiturienta=Abiturient.KodAbiturienta 

                    query.append(" PR.KodAbiturienta=Abiturient.KodAbiturienta AND AbitDopInf.KodAbiturienta=Abiturient.KodAbiturienta AND Forma_Obuch.KodFormyOb=Abiturient.KodFormyOb AND Osnova_Obuch.KodOsnovyOb=Abiturient.KodOsnovyOb AND Gruppy.KodGruppy=Abiturient.KodGruppy AND Spetsialnosti.KodFakulteta = Fakultety.KodFakulteta AND Abiturient.KodPunkta = Punkty.KodPunkta AND Abiturient.KodRajona = Rajony.KodRajona AND Abiturient.KodOblasti = Oblasti.KodOblasti AND Abiturient.KodLgot = Lgoty.KodLgot AND Abiturient.KodMedali = Medali.KodMedali AND Abiturient.KodZavedenija = Zavedenija.KodZavedenija AND Abiturient.KodKursov = Kursy.KodKursov AND TselevojPriem.KodTselevogoPriema=Abiturient.KodTselevogoPriema AND Abiturient.KodVuza = ");
...
//System.out.println(">>"+session.getAttribute("Special1"));
                  ...
                  /////////////////
                   ...
                   if(!(session.getAttribute("PreemptiveRight")+"").equals("%"))
                	   ...
                	   //condition.append(" AND PreemptiveRight.PreemptiveRight IN("+session.getAttribute("PreemptiveRight")+") "+" OR PreemptiveRight.PreemptiveRight IS NULL");
                       //condition.append(" AND PreemptiveRight.PreemptiveRight LIKE "+"'"+session.getAttribute("PreemptiveRight")+"'"+" OR PreemptiveRight.PreemptiveRight IS NULL");
                	   //condition.append(" AND PreemptiveRight.PreemptiveRight LIKE "+"'"+session.getAttribute("PreemptiveRight")+"' "+" OR PreemptiveRight.PreemptiveRight IS NULL");//LIKE "+"'"+session.getAttribute("ProvidingSpecialCondition")+"'
                	   //condition.append(" AND PreemptiveRight.PreemptiveRight LIKE "+"'"+session.getAttribute("PreemptiveRight")+"'"+" OR PreemptiveRight.PreemptiveRight IS NULL");
                	   //condition.append(" AND PreemptiveRight.PreemptiveRight IN("+session.getAttribute("PreemptiveRight")+") "+" OR PreemptiveRight.PreemptiveRight IS NULL");
                   //if((session.getAttribute("PreemptiveRight")+"").equals("%"))
                	//   condition.append(" AND PreemptiveRight.PreemptiveRight IN("+session.getAttribute("PreemptiveRight")+") "+" OR PreemptiveRight.PreemptiveRight IS NULL");
                       //condition.append(" AND PreemptiveRight.PreemptiveRight IN("+session.getAttribute("PreemptiveRight")+") "+" OR PreemptiveRight.PreemptiveRight IS NULL");
                      // condition.append(" AND PreemptiveRight.PreemptiveRight LIKE '"+session.getAttribute("PreemptiveRight")+"'");
                    //else
                      // condition.append(" AND PreemptiveRight.PreemptiveRight LIKE "+"'"+session.getAttribute("PreemptiveRight")+"'");
                   ...
                   if(!(session.getAttribute("ProvidingSpecialConditions")+"").equals("%"))
                	   condition.append(" AND AbitDopInf.ProvidingSpecialConditions IS NOT NULL");//"+" OR AbitDopInf.ProvidingSpecialConditions IS NULL
                	   //condition.append(" AND AbitDopInf.ProvidingSpecialCondition IN("+session.getAttribute("ProvidingSpecialCondition")+") "+" OR AbitDopInf.ProvidingSpecialCondition IS NULL");//LIKE "+"'"+session.getAttribute("ProvidingSpecialCondition")+"'
                       //condition.append(" AND AbitDopInf.ProvidingSpecialConditions LIKE '"+session.getAttribute("ProvidingSpecialCondition")+"'");
                    // else
                       //condition.append(" AND AbitDopInf.ProvidingSpecialConditions LIKE "+"'"+session.getAttribute("ProvidingSpecialCondition")+"'");
                   /////////////////////////////
...
/// System.out.println(query);
                    ...
                    StringBuffer queryTotal = new StringBuffer("SELECT COUNT(Abiturient.KodAbiturienta) FROM Forma_Obuch,Osnova_Obuch,TselevojPriem,Abiturient,Lgoty,Medali,Kursy,Spetsialnosti,Fakultety,Punkty,Rajony,Oblasti,Zavedenija,Gruppy,PR,AbitDopInf");
                    ...
                    queryTotal.append(" PR.KodAbiturienta=Abiturient.KodAbiturienta AND AbitDopInf.KodAbiturienta=Abiturient.KodAbiturienta AND Forma_Obuch.KodFormyOb=Abiturient.KodFormyOb AND Osnova_Obuch.KodOsnovyOb=Abiturient.KodOsnovyOb AND Gruppy.KodGruppy=Abiturient.KodGruppy AND Spetsialnosti.KodFakulteta = Fakultety.KodFakulteta AND Abiturient.KodPunkta = Punkty.KodPunkta AND Abiturient.KodRajona = Rajony.KodRajona AND Abiturient.KodOblasti = Oblasti.KodOblasti AND Abiturient.KodLgot = Lgoty.KodLgot AND Abiturient.KodMedali = Medali.KodMedali AND Abiturient.KodZavedenija = Zavedenija.KodZavedenija AND Abiturient.KodKursov = Kursy.KodKursov AND TselevojPriem.KodTselevogoPriema=Abiturient.KodTselevogoPriema AND Abiturient.KodVuza=");
...
// Начало выборки
   ...
   if(rs.next()) {
     ...
     abit_TMP.setNomerPlatnogoDogovora(rs.getString(11));
     abit_TMP.setDataRojdenija(rs.getString(12));
     abit_TMP.setPol(rs.getString(13));
     abit_TMP.setSrokObuchenija(rs.getString(14));
     abit_TMP.setGodOkonchanijaSrObrazovanija(new Integer(rs.getString(15)));
     abit_TMP.setGdePoluchilSrObrazovanie(rs.getString(16));
     abit_TMP.setNomerShkoly(rs.getString(17));
     abit_TMP.setInostrannyjJazyk(rs.getString(18));
     abit_TMP.setNujdaetsjaVObschejitii(rs.getString(19));
     abit_TMP.setGrajdanstvo(rs.getString(20));
     ...
     abit_TMP.setPolnoeNaimenovanieZavedenija(rs.getString(24));
     abit_TMP.setTipOkonchennogoZavedenija(rs.getString(25));
     abit_TMP.setTrudovajaDejatelnost(rs.getString(26));
     abit_TMP.setGruppa(rs.getString(27));
     abit_TMP.setNapravlenieOtPredprijatija(rs.getString(28));
     abit_TMP.setTipDokumenta(rs.getString(29));
     abit_TMP.setNomerDokumenta(rs.getString(30));
     abit_TMP.setSeriaDokumenta(rs.getString(31));
     abit_TMP.setDataVydDokumenta(rs.getString(32));
     abit_TMP.setKemVydDokument(rs.getString(33));
     abit_TMP.setTipDokSredObraz(rs.getString(34));
     abit_TMP.setSobesedovanie(rs.getString(35));
     abit_TMP.setShifrPriema(rs.getString(36));
     abit_TMP.setNomerSertifikata(rs.getString(37));
     abit_TMP.setKopijaSertifikata(rs.getString(38));
     abit_TMP.setBall(new Integer(rs.getInt(39)));
     abit_TMP.setPrinjat(rs.getString(40));
     abit_TMP.setFormaOb(rs.getString(42));
     abit_TMP.setOsnovaOb(rs.getString(43));
     abit_TMP.setSeriaAtt(rs.getString(44));
     abit_TMP.setNomerAtt(rs.getString(45));
     abit_TMP.setPreemptiveRight(rs.getString(46));
     abit_TMP.setProvidingSpecialCondition(rs.getString(47));  
     abit_TMP.setDopAddress(rs.getString(48));
     abit_TMP.setReturnDocument(rs.getString(49));
...
// KodSpetsialnosti (Зачисления)
   stmt_a.setObject(2,rs.getString(41),Types.INTEGER);

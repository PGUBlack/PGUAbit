...
public class SearchRTFOtsAction extends Action {
             ...
             if(curr_code == StringUtil.toInt(MassOtsForCol[j][0],-1)) {
              ...
              System.out.println("SELECT "+MassOtsForCol[j][2]+" FROM Otsenki,ZajavlennyeShkolnyeOtsenki WHERE Otsenki.KodAbiturienta=ZajavlennyeShkolnyeOtsenki.KodAbiturienta AND Otsenki.KodPredmeta=ZajavlennyeShkolnyeOtsenki.KodPredmeta AND Otsenki.KodPredmeta LIKE "+MassOtsForCol[j][0]+" AND Otsenki.KodAbiturienta LIKE "+ rs.getString(1));
               ...
               if(rs_a.next()) {
            	//   colIndex++;
            	 //  Cell cell = cells.get(rowIndex,colIndex);
            		// cell.setValue(rs_a.getString(1));
            	   report.write("\\intbl\\qc{"+StringUtil.voidFilter(rs_a.getString(1))+"}\\cell\n");
               }
...
//########################################################################################
    ...
    /*   String ename = "Результаты поиска по оценкам от "+StringUtil.CurrDate(".")+" на "+StringUtil.CurrTime(":");

       String efile_con = new String("srch_ots_d_"+StringUtil.CurrDate(".")+"_t_"+StringUtil.CurrTime("_"));

      // session.setAttribute("rpt",StringUtil.AddToRepBrw(user.getName()+user.getUid(),name,file_con,"xls"));

       String efile_name = (request.getRealPath(request.getContextPath())).substring(0,request.getRealPath(request.getContextPath()).lastIndexOf('\\'))+Constants.RELATIVE_PATH+"\\"+"monitoring.xls";

       BufferedWriter ereport = new BufferedWriter(new FileWriter(file_name)); 
